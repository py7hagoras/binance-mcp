      ...
      try {
        switch (request.params.name) {
          ...
          case 'get_funding_wallet': {
            const { asset, needBtcValuation } = request.params.arguments as {
              asset?: string;
          ...
          case 'get_simple_earn_flexible_positions': {
            const positions = await this.binanceApi.getSimpleEarnFlexiblePositions(
          ...
          case 'get_simple_earn_locked_positions': {
            const positions = await this.binanceApi.getSimpleEarnLockedPositions(
              request.params.arguments as any
            );
            return {
              content: [{ type: 'text', text: JSON.stringify(positions, null, 2) }],
          ...
          case 'get_isolated_margin_account': {
            const { symbols } = request.params.arguments as { symbols?: string };
